...
function MenShirt() {
  ...
  useEffect(() => {
    const mensShirtFetcher = async() => {
      const response = await axios.get('http://localhost:3000/api/v1/products/category/Men-Shirt');
      console.log(response.data.categorySpecificProducts);
      const menShirt = response.data.categorySpecificProducts;
      setProducts(menShirt);
    }
    mensShirtFetcher();
    const handleResize = () => {
