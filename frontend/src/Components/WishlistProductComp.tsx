    ...
    <>
      <div className="flex border-2 rounded-md">
        <Link to={`/product/${title.replace(/\s+/g, "-").toLowerCase()}`}>
          <img className="h-full w-[75%]" src={imgSrc} alt="img" />
        </Link>
        <div className="w-full flex space-y-1 py-2 px-1 flex-col justify-start">
          ...
          <div className="flex flex-row items-center py-2">
            <span className="font-semibold text-sm md:text-xl">₹{newPrice}/-</span>
            <span className="ml-2 text-sm text-neutral-500">
              <span className="text-[11px] mx-0.5">M.R.P:</span>
              <s>₹{oldPrice}/-</s>
            </span>
          </div>
          <span className="text-sm font-light md:text-xl">FREE Delivery By CartCraze</span>
          <div className="w-full flex flex-row space-x-5 items-center pt-4">
            <button
              onClick={(e) => e.stopPropagation()}
              className="bg-gray-800 hover:bg-black text-white flex justify-center p-1 w-[30vw] rounded-2xl text-sm lg:w-48"
            >
              Add to cart
            </button>
            <button
              onClick={(e) => e.stopPropagation()}
              className="hidden lg:flex border-2 border-black hover:border-red-600 hover:text-red-600 justify-center p-1 w-[30vw] rounded-2xl text-sm lg:w-48"
            >
